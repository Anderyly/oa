layui.config({
	...
	$(function() {
		...
		winui.config({
			...
			menu: {
				options: {
					url: sessionStorage.getItem("url") + 'user/getMenu1',
	...
	//开始菜单左侧苏北评估智地规划办公系统按钮点击
	$('.winui-start-item.winui-start-user').on('click', function() {
		winui.window.openUser();
	});
	...
	$('.logout').on('click', function() {
		...
		winui.window.confirm('确认注销吗?', {
			...
			$.ajax({
				url: sessionStorage.getItem("url") + "user/logout",
				type: sessionStorage.getItem("method"),
				dataType: 'json',
				data: {token: sessionStorage.getItem("token")},
				async: false,
				success: function(data) {
					if (data.code == 200 || data.code == 1001 || data.code == 1003 || data.code == 1002) {
	...
	// 锁屏调用
	function lock() {
		winui.lockScreen(function(account, password) {
			$.ajax({
				url: sessionStorage.getItem("url") + "user/login",
				type: sessionStorage.getItem("method"),
				dataType: 'json',
				data: {
					account: account,
	...
	// 判断是否显示锁屏（这个要放在最后执行）
	if (window.localStorage.getItem("lockscreen") == "true" || sessionStorage.getItem("token") == undefined) {
		return lock()
	}
