...
layui.define(['jquery', 'layer', 'winui'], function(exports) {
	...
	var $ = layui.jquery
		//常量字符串
		,
		SETWIN = '.winui-helper-setwin',
		LOCK = '.winui-helper-lock',
		MENU = '.winui-helper-menu',
		SWITCH = ".winui-helper-switch",
		CONTENT = '.winui-helper-content',
		TOOL = '.winui-helper-tool'
		//16进制颜色校验表达式
		,
		...
		addTool = function(tips, icon, style) {
			return '<span class="tool-tags" data-tips="' + tips + '"><i ' + (style ? 'style = "' + style + '"' :
				'') + ' class="fa fa-fw ' + icon + '"></i></span>';
		}
		//桌面助手的HTML字符
		,
		...
		getSetwin = function(lock, up) {
			return '<span class="winui-helper-setwin"><a title="解锁" class="winui-helper-lock"><i class="fa fa-fw ' +
				(lock ? 'fa-lock' : 'fa-unlock-alt') +
				'"></i></a><a title="菜单" class="winui-helper-menu"><i class="fa fa-fw fa-bars"></i></a><a title="收起" class="winui-helper-switch"><i class="fa fa-fw ' +
				(up ? 'fa-chevron-up' : 'fa-chevron-down') + '"></i></a></span>';
		},
		toggleHide = function() {
	...
	Class.prototype.render = function(options, callback) {
		options = options || (layui.data('winui-helper').options || {
			move: true,
			lock: false,
			up: false
		});
		...
		var configs = {
			...
			success: function(layero, index) {
				$(layero).css({
					'background-color': hexToRGBA(that.config.bgColor, that.config.opacity)
				});
				//渲染右上角
				...
				that.addTool([{
					...
					click: function(e) {
						...
						$(CONTENT).append(
							'<hr /><div class="tags-content"><textarea placeholder="输入内容，如：桌面助手真牛逼"></textarea></div>'
							);
						var $currTextarea = $(CONTENT).children('.tags-content').eq(
							tagsCount).children('textarea');
						$currTextarea.focus();
						$currTextarea.on('blur', function() {
							...
							if ($.trim(content) === '') {
								...
								$(this).parent().remove();
								var tags = layui.data('winui-helper')
									.tags || [];
				...
				if (tags instanceof Array)
					$.each(tags, function(index, item) {
						if (index >= 5)
							return;
						$(CONTENT).append(
							'<hr /><div class="tags-content"><textarea placeholder="输入内容，便签存于本地">' +
							item + '</textarea></div>');
						$(CONTENT).children('.tags-content').eq(index).children('textarea').on(
							'blur',
							function() {
								var content = $(this).val();
								var tags = layui.data('winui-helper').tags || [];
				...
				$(MENU).on('click', function(e) {
					...
					var left = e.clientX,
						top = e.clientY,
						html = '<ul class="helper-menu" style="top:' + top + 'px;left:' +
						left +
						'px;"><li><i class="fa fa-fw fa-cog"></i>设置</li><li><i class="fa fa-fw fa-outdent"></i>退出桌面助手</li></ul>';
					//移除之前菜单
					$('.helper-menu').remove();
					//渲染当前菜单
					$('body').append(html);
					...
					$('.helper-menu li').eq(0).on('mousedown', function(e) {
						layui.stope(e);
						if (e.button == 0) {
							$.get(winui.path + 'html/helper/settings.html', {},
								function(res) {
									layer.open({
										id: 'helpersettings',
										title: '桌面助手设置中心',
				...
				$(SWITCH).on('click', function(e) {
					...
					$(SWITCH).find('.fa').toggleClass('fa-chevron-up').toggleClass(
						'fa-chevron-down');
	...
	Class.prototype.addTool = function(options) {
		...
		$.each(options, function(index, item) {
			//拼接HTML
			$(TOOL).append('<span data-tips="' + item.tips + '"><i ' + (item.style ? 'style = "' +
				item.style + '"' : '') + ' class="fa fa-fw ' + item.icon + '"></i></span>');
			//绑定事件
			...
			tipIndex = layer.tips($(this).data('tips'), this, {
				...
				success: function(layero) {
					var top = Number($(layero).css('top').replace('px', '')) + 5 + 'px',
						left = Number($(layero).css('left').replace('px', '')) - 10 +
						'px';
					$(layero).css({
						'top': top,
						'left': left
